...
var server_commands = preload("res://ServerCommands.gd").new()
...
func Init():
	...
	#        SETTERS
	# ----------------------
	Console.create_command("set_credits", GlobalVariables.captain_container.SetCredits, "Sets the given captain's credits to the given amount. \nE.X: set_client_credits [captain_id : int] [amount : int]")
	
	# ----------------------
	#  MULTIPLAYER COMMANDS
	# ----------------------
	Console.create_command("op", server_commands.OpCommand, "Adds a player based on the given id to the admin team, allowing them to execute server console commands. \nE.X: op [captain_id : int]")
	Console.create_command("kick", server_commands.KickCommand, "Kicks a player based on the given id from the game. \nE.X: kick [captain_id : int]")
	# ----------------------
	#     SHIP COMMANDS
	# ----------------------
	Console.create_command("change_ship", GameManager.ChangePlayerShip, "Changes the player ship to the ship with the given id. \nE.X: change_ship [id : int]")
	...
	Console.create_command("tp", GameManager.Teleport, "Teleports the ship with the given id to the given coordinates. \nE.X: tp [id : int] [x : int] [z : int]")
	Console.create_command("warp", GameManager.WarpCommand, "Warps the given ship to the given system. \nE.X: warp [ship_id : int] [system_id : int]")
	...
	Console.create_command("join_faction", GlobalVariables.faction_container.JoinFactionCommand, "Adds the given captain as a member of the given faction. \nE.X: join_faction [faction_name : String] [captain_id : int]")
	...
	Console.create_command("ships", GameManager.GetShipsCommand, "Gets all the ships in the game.")
	...
	Console.create_command("captains", GlobalVariables.captain_container.GetCaptainsCommand, "Gets all the active captains in the scene.")
	Console.create_command("starbases", GameManager.GetStarbasesCommand, "Gets all the starbases in the scene.")
	Console.create_command("factions", GlobalVariables.faction_container.GetFactionsCommand, "Gets all the factions in the scene.")
