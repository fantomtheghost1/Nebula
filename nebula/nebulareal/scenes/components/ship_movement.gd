...
func Init(ship_node_param : Ship):
	ship_node = ship_node_param

func _process(_delta : float):
	if is_moving:
		SendTransform.rpc(ship_node.position, ship_node.rotation)
...
func PathTo(target_pos : Vector3, number_of_waypoints : int, distance_between_waypoints: float, inaccuracy_coefficient: float):
	...
	var index = 0
	var _previous_waypoint_pos = (target_pos * (index) / number_of_waypoints)
...
func MoveShip():
	...
	# rotates ship towards the next waypoint in the queue
	if current_waypoint:
		if ship_node.position != current_waypoint.position:
			ship_node.look_at(current_waypoint.position)
...
func SendTransform(pos : Vector3, rot : Vector3):
	ship_node.position = pos
	ship_node.rotation = rot
