...
func Initialize(resource : Resource, starbase_id : int, system_name_param : String = "") -> void:
	...
	%WarehouseComponent.CreateWarehouse()
	#%IdentityComponent.SetOwner.rpc(CaptainManager.CreateAICaptain())
...
func _on_deposit_materials_pressed() -> void:
	var warehouse = %WarehouseComponent.GetWarehouseFromCaptain(GlobalVariables.client_captain)
	if !%DockComponent.docked_ships[0].cargo_comp.IsCargoHoldEmpty():
		var new_cargo = %DockComponent.docked_ships[0].cargo_comp.GetCargo()
		%DockComponent.docked_ships[0].cargo_comp.ClearCargo()
		for item in new_cargo:
			warehouse.AddCargo(new_cargo[item].item, new_cargo[item].quantity)
	else:
		print_rich("[color=yellow]Cargo Bay Is Empty!")
